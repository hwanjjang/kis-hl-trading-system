...
def kis_overseas(row):
    ...
    return 'trade',key,dict(instrument='kis:'+row['pdno'],symbol=row['pdno'],currency=row['crcy_cd'],
        ...
        price=number(amount/D(row['ccld_qty'])),costs={'broker':number(broker),'overseas':number(foreign),'tax':None,'interest':None},
...
def domestic_bundle(bundle):
    ...
    for day in bundle['days']:
        ...
        for code,prefix in [('02','buy'),('01','sll')]:
            ...
            if len(rows)!=1:raise ValueError('Shared daily commission requires explicit allocation')
            r=rows[0];fee=D(number(summary[prefix+'_fee_smtl']));tax=D(number(summary['buy_tax_smtl' if code=='02' else 'sll_tltx_smtl']))
            start,end=day_interval(day['trad_dt'],'Asia/Seoul');amount=D(number(r['tot_ccld_amt']));quantity=D(number(r['tot_ccld_qty']))
            result.append(dict(source_id=encode([r['ord_dt'],r['pdno'],code,r['odno']]),instrument='kis:'+r['pdno'],symbol=r['pdno'],currency='KRW',
                event_start_ms=start,event_end_ms=end,time_precision='DAY',grain='ORDER_CUMULATIVE_RECONCILED',side='buy' if code=='02' else 'sell',
                ...
                settlement=number(amount+(fee+tax)*(1 if code=='02' else -1)),order_time=r.get('ord_tmd'),day_end_quantity=number(day['hldg_qty']),strategy='unassigned',origin='unknown'))
