...
- Use the explicitly selected country exposure without duplicate trade candidates:
  - `KORU` (`xyz:KORU`) is the preferred South Korea exposure; `KR200` and `EWY` are retained but excluded, with `preferred_symbol = KORU`.
...
The user-selected `KORU` market references the Direxion Daily MSCI South Korea
Bull 3X ETF, a leveraged ETF targeting 300% of its benchmark daily performance
before fees and expenses. It is not an equivalent replacement for KR200 or
KOSPI200: benchmark, daily leverage/reset, compounding and trading session differ.
The ETF uses the U.S. cash-equity session, not KRX hours.

The execution instrument is `hl:xyz:KORU`. Its KIS mapping is a quote-only
`overseas` / `AMS` / `KORU` route generated by the existing ETF mapper; no KIS
execution instrument is added. Metadata freshness and all live safety guards
still apply. This policy change does not establish live order acceptance or
native trailing-stop validation.

References: [trade.xyz specification index](https://docs.trade.xyz/consolidated-resources/specification-index),
[Direxion KORU](https://www.direxion.com/product/daily-msci-south-korea-bull-3x-etf).
...
- `trade_symbol`: canonical project symbol, such as `SAMSUNG`, `SKHYNIX`, or `KORU`.
...
- `KORU` uses the KIS overseas quote endpoint with exchange code `AMS` and symbol `KORU`.
- `KR200` retains the domestic index route with index code `2001` for auditability, but its mapping is `excluded`.
...
- `EWY` and `EWJ` retain quote mappings for auditability but are `excluded` because this project selects `KORU` and `JP225` instead.
...
- Active index fallbacks use `^GSPC`, `^NDX`, and `^N225`; the excluded KR200 row retains `^KS200`. KORU historical bars use the ETF ticker `KORU`.
